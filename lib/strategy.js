 ...
 * @property {string} [domain="https://forgehub.com"]
 * @property {string} [authorizationURL="https://forgehub.com/api/oauth2/authorize"]
 * @property {string} [tokenURL="https://forgehub.com/api/oauth2/token"]
 * @property {string} [scopeSeparator=" "]
 */
/**
 * `Strategy` constructor.
 *
 * The Forgehub authentication strategy authenticates requests by delegating to
 * Forgehub via the OAuth2.0 protocol
 *
 ...
 *
 * Options:
 *   - `clientID`       OAuth ID to forgehub
 *   - `clientSecret`   OAuth Secret to verify client to forgehub
 *   - `callbackURL`    URL that forgehub will redirect to after auth
...
function Strategy(options, verify) {
    options = options || {};
    domain = options.domain || 'https://forgehub.com';
    options.authorizationURL = options.authorizationURL || (domain + '/api/oauth2/authorize');
    options.tokenURL = options.tokenURL || (domain + '/api/oauth2/token');
...
/**
 * Retrieve user profile from Forgehub.
 *
...
Strategy.prototype.userProfile = function(accessToken, done) {
    var self = this;
    this._oauth2.get(domain + '/api/users/me', accessToken, function(err, body, res) {
        if (err) {
...
Strategy.prototype.checkScope = function(scope, accessToken, cb) {
    if (this._scope && this._scope.indexOf(scope) !== -1) {
        this._oauth2.get(domain + '/api/users/me/' + scope, accessToken, function(err, body, res) {
